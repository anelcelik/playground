...
## 2. Generate the iOS project files + add native Swift files

On Mac, `cd` into the `IOS/` folder and run:

```bash
flutter create \
  --project-name playground_tracker \
  --org com.playground.tracker \
  --platforms ios \
  .
```

This generates the `ios/` folder with the Xcode project inside.
It will **not** overwrite your existing `lib/` source files.
...
Then install iOS CocoaPods dependencies:
```bash
cd ios && pod install && cd ..
```

Then copy the CloudKit Swift files into the Xcode project:
```bash
cp swift/CloudKitPlugin.swift ios/Runner/CloudKitPlugin.swift
cp swift/AppDelegate.swift     ios/Runner/AppDelegate.swift
```

Then open Xcode and **add CloudKitPlugin.swift to the Runner target**:
1. Open `ios/Runner.xcworkspace` in Xcode
2. In the Project Navigator, right-click `Runner/Runner` folder → **Add Files to "Runner"**
3. Select `ios/Runner/CloudKitPlugin.swift`
4. Make sure **"Add to targets: Runner"** is checked → click Add
5. Do the same for `AppDelegate.swift` if Xcode shows it as not in the target

⚠️ `AppDelegate.swift` was already generated by `flutter create` — the copy step above replaces it with our modified version that registers the CloudKit plugin. If you've made custom changes to AppDelegate, merge them manually.
