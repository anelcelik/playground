...
/// Square toggle.
///
/// Material's `Switch` is a capsule by construction — it has no shape hook,
/// so a system with no radii anywhere needs its own. Same semantics and the
/// same 44px hit target; `null` onChanged disables it.
class SquareSwitch extends StatelessWidget {
  final bool value;
  final ValueChanged<bool>? onChanged;
  const SquareSwitch({super.key, required this.value, this.onChanged});
          ...
          child: SizedBox(
            ...
            child: Center(
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: value ? c.green : Colors.transparent,
                  border: Border.all(
                      color: value ? c.green : c.hairline, width: 2),
                ),
                child: AnimatedAlign(
                  duration: const Duration(milliseconds: 120),
                  alignment:
